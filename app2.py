...
# Function Heart Disease Prediction (VERSI BARU)
def heart():
    st.write("""
        Halaman ini berguna untuk memprediksi apakah seseorang terindikasi memiliki Penyakit Jantung  
        Prediksi ini berdasarkan dataset dari [Heart Disease dataset](https://archive.ics.uci.edu/dataset/45/heart+disease) by UCIML.
        Hasil yang didapatkan sangat memungkinkan berbeda dengan prediksi menggunakan data atau teknik lain. Konsultasi ke Dokter
        Spesialis Jantung adalah tindakan utama yang harus dilakukan. 
        """)
    
    # --- 1. MEMPERBAIKI INPUT SIDEBAR ---
    # Kita harus mengumpulkan SEMUA 13 fitur yang digunakan untuk melatih model
    
    st.sidebar.header('User Input Features:')
    
    # --- Kolom Numerik (HARUS SAMA PERSIS DENGAN 'numeric_cols' di Colab) ---
    ...
    thalach = st.sidebar.slider("Maximum Heart Rate (thalach)", 71, 202, 152)
    oldpeak = st.sidebar.slider("ST Depression (oldpeak)", 0.0, 6.2, 0.8)
    
    # --- BLOK KODE DUPLIKAT DI BAWAH INI SUDAH DIHAPUS ---
    ...
    # --- Kolom Kategorikal (HARUS SAMA PERSIS DENGAN 'lst' di Colab, TANPA 'target') ---
    # Ini adalah satu-satunya blok yang kita perlukan
    sex = st.sidebar.selectbox("Sex", (1, 0), format_func=lambda x: 'Male' if x == 1 else 'Female', key='sex_input')
    cp = st.sidebar.selectbox("Chest Pain Type (cp)", (0, 1, 2, 3), format_func=lambda x: {0: 'Typical Angina', 1: 'Atypical Angina', 2: 'Non-anginal Pain', 3: 'Asymptomatic'}[x], key='cp_input')
    fbs = st.sidebar.selectbox("Fasting Blood Sugar > 120 mg/dl (fbs)", (1, 0), format_func=lambda x: 'Yes' if x == 1 else 'No', key='fbs_input')
    restecg = st.sidebar.selectbox("Resting ECG (restecg)", (0, 1, 2), key='restecg_input')
    exang = st.sidebar.selectbox("Exercise Induced Angina (exang)", (1, 0), format_func=lambda x: 'Yes' if x == 1 else 'No', key='exang_input')
    slope = st.sidebar.selectbox("Slope", (0, 1, 2), format_func=lambda x: {0: 'Downsloping', 1: 'Flat', 2: 'Upsloping'}[x], key='slope_input')
    ca = st.sidebar.slider("Number of Major Vessels (ca)", 0, 4, 0, key='ca_input') # Slider juga butuh key jika duplikat
    thal = st.sidebar.selectbox("Thal", (1, 2, 3), format_func=lambda x: {1:'Normal', 2:'Fixed Defect', 3:'Reversable Defect'}[x], key='thal_input')

    # Kumpulkan data dalam DataFrame
    data = {'age': age,
            'trestbps': trestbps,
            'chol': chol,
            'thalach': thalach,
    ...
    #st.subheader("Data Input Pengguna (Mentah)")
    st.dataframe(input_df)

    # --- 2. MEMPERBAIKI PROSES PREDIKSI ---
    if st.sidebar.button("GO!"):
        try:
            # Muat SEMUA artefak (model, scaler, dan pca)
            model_path = 'model/random_forest.joblib'
            scaler_path = 'model/scaler.joblib'
            pca_path = 'model/pca.joblib'
            
            model = joblib.load(model_path)
            scaler = joblib.load(scaler_path)
            pca = joblib.load(pca_path)
            
            # Tentukan urutan kolom PERSIS seperti saat training df_combined
            numeric_cols = ['age', 'trestbps', 'chol', 'thalach', 'oldpeak']
            categorical_cols = ['sex', 'cp', 'fbs', 'restecg', 'exang', 'slope', 'ca', 'thal']
            
            # --- Jalankan Pipeline Preprocessing ---
            
            df_num = input_df[numeric_cols]
            df_cat = input_df[categorical_cols]

            df_num_scaled = scaler.transform(df_num)
            df_num_scaled_df = pd.DataFrame(df_num_scaled, columns=numeric_cols)

            df_combined_input = pd.concat([df_num_scaled_df.reset_index(drop=True), 
                                           df_cat.reset_index(drop=True)], axis=1)

            df_pca_input = pca.transform(df_combined_input) # Ini menghasilkan 9 fitur
            
            # --- INI ADALAH PERBAIKAN ERROR 9 vs 8 ---
            # Kita 'slice' array NumPy untuk mengambil 8 fitur pertama
            # agar sesuai dengan apa yang diharapkan model
            df_pca_input_sliced = df_pca_input[:, :8] 

            # 5. Prediksi dengan data 8-fitur yang sudah di-slice
            prediction = model.predict(df_pca_input_sliced)
            ...
            with st.spinner('Sedang memproses...'):
                ...
                if prediction[0] == 0:
                    # Jika hasilnya 0 (Aman)
                    st.success('Pasien ini tidak terindikasi memiliki penyakit jantung')
                    st.balloons()
                else:
                    # Jika hasilnya 1 (Berisiko)
                    st.error('Pasien ini terindikasi memiliki penyakit jantung')
                
        except FileNotFoundError:
            st.error(f"ERROR: File model/scaler/pca tidak ditemukan.")
            st.error(f"Pastikan 'random_forest.joblib', 'scaler.joblib', dan 'pca.joblib' ada di folder 'model/'.")
        except Exception as e:
            st.error(f"Terjadi kesalahan: {e}")
            st.error("Pastikan urutan kolom di 'numeric_cols' dan 'categorical_cols' sudah benar.")
...
if nav == "Home":
    ...
    **3. Dimensionality Reduction**  
    ...
    st.header("Hasil Prediksi untuk Kamu")
    heart()

elif nav == "About":
    st.title("About Me")  
    st.write('''
    Saya Pujo Satrianto, kelahiran Kutai Kartanegara namun bersuku Jawa. Saat ini saya merupakan seorang ASN pada salah satu
    instansi kementerian di Indonesia. Latar belakang pendidikan saya ialah S1 Ilmu Komputer pada Universitas Mulawarman
    Samarinda, lulus pada tahun 2013 dengan GPA 3.67/4.00. Untuk lebih jelas, di bawah ini link sosial media saya. Salam kenal
    semuanya dan selamat beraktifitas.  
    
    LinkedIn : [Pujo Satrianto](www.linkedin.com/in/pujo-satrianto-11741912a)  
    Instagram : [@pujosatrianto](https://www.instagram.com/pujosatrianto/)  
    Whatsapp : [08377385544](https://wa.me/6285347385544)
    ''')
